...
class KelasPelatihanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('pelatihan.index')->with('success', 'Kendaraan berhasil ditambahkan!');
    }
...
public function edit($id)
{
    ...
    public function kirim($id, $id_kelas_pengemudi)
    {
        // Update jumlah kelas lulus
        Pengemudi::where('id_pengemudi', $id)
            ->update([
                'jumlah_kelas_lulus' => DB::raw('jumlah_kelas_lulus + 1')
            ]);
            $id_kelas = kelas_pengemudi::where('id_pengemudi', $id)->value('id_kelas');

        kelas_pengemudi::where('id_pengemudi', $id)
            ->where('id_kelas_pengemudi', $id_kelas_pengemudi)
            ->delete();
 
            $jumlah_peserta = kelas_pengemudi::where('id_kelas', $id_kelas)->count();
            kelas_pengemudi::where('id_pengemudi', $id)
            ->where('id_kelas_pengemudi', $id_kelas_pengemudi)
            ->delete();

            kelas_pelatihan::where('id_kelas', $id_kelas)->update([
                'jumlah_peserta' => $jumlah_peserta
            ]);
            
          
    
        return route('pelatihan.edit',[$id => $id]);
    }
}
