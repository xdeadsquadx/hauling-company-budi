...
class MaintenanceController extends Controller
{
    ...
    public function status($id)
    {
        ...
        Maintenance::where('id_kendaraan', $id)->update(['status' => 1]);
    
        // Update status kendaraan
        Kendaraan::where('id_kendaraan', $id)->update(['status_maintenance' => 1],['terakhir_maintenance'] => Kendaraan::where('id_kendaraan', $id)->update(['status_maintenance' => 1]););
    
        return redirect()->route('maintenance.index')->with('success', 'Kendaraan berhasil diperbarui!');
    }
